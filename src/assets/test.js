...
+function ($) {
    ...
    // 自定义modal
    // callbackList[{className:"className", func:"function"}]
    $.customModal = function(innerHtml, callbackList){
      var modal = $('<div class="modal">'+ innerHtml +'</div>')
      $(defaults.modalContainer).append(modal[0]);
      $.each(callbackList, function(index, item){
        modal.find('.'+item.className).on("click", item.func);
      })
      $.openModal(modal);
      return modal[0]
    }
